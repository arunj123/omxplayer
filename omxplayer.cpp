...
#define DISPLAY_TEXT(text, ms) \
    if (m_osd)                 \
    m_player_subtitles.DisplayText(text, ms)
...
constexpr int S(double x) { return (int)(DVD_PLAYSPEED_NORMAL * (x)); }

struct OMXPlayer_t
{
    enum PCMChannels *m_pChannelMap {nullptr};
    ...
    static constexpr int playspeeds[30]{S(0), S(1 / 16.0), S(1 / 8.0), S(1 / 4.0), S(1 / 2.0), S(0.975), S(1.0), S(1.125), S(-32.0), S(-16.0), S(-8.0), S(-4), S(-2), S(-1), S(1), S(2.0), S(4.0), S(8.0), S(16.0), S(32.0)};
    static constexpr int playspeed_slow_min {0}, playspeed_slow_max {7}, playspeed_rew_max {8}, playspeed_rew_min {13}, playspeed_normal {14}, playspeed_ff_min {15}, playspeed_ff_max {19};
    ...
    static constexpr auto PrintFileNotFound = [](const std::string &path)
    {
    ...
    auto initialise()
    {
        ...
        control_err = m_omxcontrol.init( m_av_clock, &m_player_audio, &m_player_subtitles, &m_omx_reader, m_dbus_name);
        if (false == m_no_keys)
        {
    ...
    void startPlayer() {
        playerthread = std::thread{ runthread, this };
    }

    static void runthread( OMXPlayer_t *plr ) {
        plr->run();
    }
    ...
    void SetVideoMode(int width, int height, int fpsrate, int fpsscale, FORMAT_3D_T is3d)
    {
        ...
        if (tv_found)
        {
            ...
            printf("ntsc_freq:%d %s\n", ntsc_freq, property.param1 == HDMI_3D_FORMAT_SBS_HALF ? "3DSBS" : property.param1 == HDMI_3D_FORMAT_TB_HALF     ? "3DTB"
                                                                                                    : property.param1 == HDMI_3D_FORMAT_FRAME_PACKING ? "3DFP"
                                                                                                                                                        : "");
            sem_t tv_synced;
...
{
    OMXPlayer_t oply{};
    oply.m_config_audio.device = "omx:alsa";
    oply.m_filename = "/home/pi/dev/isignale/src/res/videos/Gemini.Man.mp4";
    oply.m_config_video.aspectMode = 3;
    oply.m_config_video.dst_rect = CRect{ 0, 0, 1920/2, 1080/2 };
    oply.initialise();
    oply.openNewMedia();
    ...
    OMXPlayer_t oply1{};
    oply1.m_config_audio.device = "omx:alsa";
    oply1.m_filename = "/home/pi/dev/isignale/src/res/videos/Gemini.Man.mp4";
    oply1.m_config_video.aspectMode = 3;
    oply1.m_config_video.dst_rect = CRect{ 1920/2,  1080/2, 1920, 1080 };
    oply1.initialise();
    oply1.openNewMedia();
